...
export class BitfinexClient extends AbstractDexClient {
	private readonly client: ccxt.bitfinex;
	private readonly logger: CustomLogger;

	constructor() {
		super();

		this.logger = new CustomLogger('Bitfinex');

		if (
			!process.env.BITFINEX_API_KEY ||
			!process.env.BITFINEX_SECRET
		) {
			this.logger.warn('Credentials are not set as environment variable');
		}

		this.client = new ccxt.bitfinex({
